...
- (void)updateBoard {
  CGFloat posX = [[[NSUserDefaults standardUserDefaults]
      objectForKey:RCVoipFloatingBoardPosX] floatValue];
  CGFloat posY = [[[NSUserDefaults standardUserDefaults]
      objectForKey:RCVoipFloatingBoardPosY] floatValue];
  posX = posX ? posX : 30;
  posY = posY ? posY : 30;
  CGRect screenBounds = [UIScreen mainScreen].bounds;
  posX = (posX + 30) > screenBounds.size.width ? (screenBounds.size.width - 30)
                                               : posX;
  posY = (posY + 48) > screenBounds.size.height
             ? (screenBounds.size.height - 48)
             : posY;
  ...
  if ([self isVideoViewEnabledSession]) {
    if (self.callSession.callStatus == RCCallActive) {
      ...
      UILabel *videoStopTips = [[UILabel alloc]
          initWithFrame:CGRectMake(0, self.videoView.frame.size.height / 2 - 10,
...
- (UIWindow *)window {
  if (!_window) {
    ...
    CGRect screenBounds = [UIScreen mainScreen].bounds;
    posX = (posX + 30) > screenBounds.size.width
               ? (screenBounds.size.width - 30)
               : posX;
    posY = (posY + 48) > screenBounds.size.height
               ? (screenBounds.size.height - 48)
               : posY;
...
- (UIButton *)floatingButton {
  if (!_floatingButton) {
    ...
    if (self.callSession.mediaType == RCCallMediaAudio) {
      [_floatingButton
          setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/audio_min.png"]
          forState:UIControlStateNormal];
    } else {
      [_floatingButton
          setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/video_min.png"]
          forState:UIControlStateNormal];
    }
  ...
  if (paramSender.state != UIGestureRecognizerStateEnded &&
    ...
    if ([RCCallKitUtility isLandscape] &&
      ...
      if (frame.origin.y + frame.size.height >
        ...
        frame.origin.y =
            [UIScreen mainScreen].bounds.size.width - 2 - frame.size.height;
      }

      if (frame.origin.x + frame.size.width >
          [UIScreen mainScreen].bounds.size.height) {
        frame.origin.x =
            [UIScreen mainScreen].bounds.size.height - 2 - frame.size.width;
      }
    } else {
      if (frame.origin.x + frame.size.width >
          [UIScreen mainScreen].bounds.size.width) {
        frame.origin.x =
            [UIScreen mainScreen].bounds.size.width - 2 - frame.size.width;
      }

      if (frame.origin.y + frame.size.height >
          [UIScreen mainScreen].bounds.size.height) {
        frame.origin.y =
            [UIScreen mainScreen].bounds.size.height - 2 - frame.size.height;
      }
    }
    self.window.frame = frame;
  } else if (paramSender.state == UIGestureRecognizerStateEnded) {
    CGRect frame = self.window.frame;
    [[NSUserDefaults standardUserDefaults] setObject:@(frame.origin.x)
                                              forKey:RCVoipFloatingBoardPosX];
    [[NSUserDefaults standardUserDefaults] setObject:@(frame.origin.y)
                                              forKey:RCVoipFloatingBoardPosY];
...
- (void)layoutTextUnderImageButton:(UIButton *)button {
  ...
  [button setTitleColor:RongVoIPUIColorFromRGB(0x0195ff)
               forState:UIControlStateNormal];
  ...
  button.imageEdgeInsets = UIEdgeInsetsMake(
      -button.titleLabel.intrinsicContentSize.height - RCCallInsideMargin / 2,
