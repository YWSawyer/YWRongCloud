...
+ (UIImage *)getScaleImage:(UIImage *)image size:(CGSize)size {
  ...
  if (size.height >= size.width) {
    [image drawInRect:CGRectMake((size.width - size.height) / 2, 0, size.height,
                                 size.height)];
  } else {
    [image drawInRect:CGRectMake(0, (size.height - size.width) / 2, size.width,
                                 size.width)];
  }
...
+ (UIImage *)getDefaultPortraitImage {
  return [RCKitUtility imageNamed:@"default_portrait_msg"
...
+ (NSString *)getGeneralReadableString:(RCCallDisconnectReason)hangupReason {
  ...
  case RCCallDisconnectReasonCancel:
    hangupReasonString =
        NSLocalizedStringFromTable(@"VoIPCallHasCancel", @"RongCloudKit", nil);
    break;
  case RCCallDisconnectReasonReject:
    hangupReasonString =
        NSLocalizedStringFromTable(@"VoIPCallHasReject", @"RongCloudKit", nil);
    break;
  case RCCallDisconnectReasonHangup:
    hangupReasonString =
        NSLocalizedStringFromTable(@"VoIPCallHasHangup", @"RongCloudKit", nil);
    break;
  ...
  case RCCallDisconnectReasonRemoteReject:
    hangupReasonString = NSLocalizedStringFromTable(@"VoIPCallRemoteReject",
                                                    @"RongCloudKit", nil);
    break;
  case RCCallDisconnectReasonRemoteHangup:
    hangupReasonString = NSLocalizedStringFromTable(@"VoIPCallRemoteHangup",
                                                    @"RongCloudKit", nil);
    break;
  case RCCallDisconnectReasonRemoteBusyLine:
    hangupReasonString = NSLocalizedStringFromTable(@"VoIPCallRemoteBusyLine",
                                                    @"RongCloudKit", nil);
    break;
...
+ (NSString *)getReadableStringForMessageCell:
  ...
  case RCCallDisconnectReasonBusyLine:
    hangupReasonString =
        NSLocalizedStringFromTable(@"VoIPCallNoResponse", @"RongCloudKit", nil);
    break;
...
+ (NSString *)getReadableStringForCallViewController:
  ...
  case RCCallDisconnectReasonNetworkError:
    hangupReasonString = NSLocalizedStringFromTable(
        @"VoIPCallLocalNetworkError", @"RongCloudKit", nil);
    break;
  case RCCallDisconnectReasonRemoteNetworkError:
    hangupReasonString = NSLocalizedStringFromTable(
        @"VoIPCallRemoteNetworkError", @"RongCloudKit", nil);
    break;
  case RCCallDisconnectReasonRemoteBusyLine:
    hangupReasonString = NSLocalizedStringFromTable(
