...
@interface RCCallVideoMultiCallViewController () <UICollectionViewDataSource,
...
- (void)addSubUserModel:(RCCallUserCallInfoModel *)model {
  if (model.userId &&
      ![[self getAllUserIdInSubUserModel] containsObject:model.userId]) {
...
- (UILabel *)userCollectionTitleLabel {
  if (!_userCollectionTitleLabel) {
    ...
    _userCollectionTitleLabel.text = NSLocalizedStringFromTable(
        @"VoIPMultiCallUserCollectionTitle", @"RongCloudKit", nil);
...
- (UICollectionView *)userCollectionView {
  if (!_userCollectionView) {
    if (self.userCollectionViewLayout) {
      _userCollectionView = [[UICollectionView alloc]
          ...
          initWithConversationType:self.conversationType
                         ...
                         mediaType:self.mediaType
                           ...
                           success:^(NSArray *addUserIdList) {
                             [weakSelf.callSession
                                 inviteRemoteUsers:addUserIdList
                                         mediaType:weakSelf.mediaType];
                           }];

  dispatch_async(dispatch_get_main_queue(), ^{
    [weakSelf presentViewController:selectViewController
                           animated:YES
                         completion:nil];
  });
}
...
- (void)resetLayout:(BOOL)isMultiCall
  ...
  if (callStatus == RCCallIncoming || callStatus == RCCallRinging) {
    [self.inviterPortraitView
        setImageURL:[NSURL URLWithString:self.mainModel.userInfo.portraitUri]];
    self.inviterPortraitView.frame = CGRectMake(
    ...
    self.inviterNameLabel.frame = CGRectMake(
        RCCallHorizontalMargin,
        RCCallVerticalMargin * 2 + RCCallHeaderLength + RCCallInsideMargin,
  ...
  CGFloat titleY = MAX(RCCallVerticalMargin * 2 + RCCallHeaderLength +
  ...
  if (callStatus == RCCallIncoming || callStatus == RCCallRinging) {
    self.userCollectionTitleLabel.frame =
        CGRectMake(RCCallHorizontalMargin, titleY,
                   self.view.frame.size.width - RCCallHorizontalMargin * 2,
                   RCCallLabelHeight);
  ...
  if (callStatus == RCCallIncoming || callStatus == RCCallRinging) {
    self.userCollectionView.frame = CGRectMake(
        RCCallHorizontalMargin * 2.5,
        titleY + RCCallLabelHeight + RCCallInsideMargin,
        self.view.frame.size.width - RCCallHorizontalMargin * 5,
        self.view.frame.size.height - RCCallVerticalMargin -
            RCCallButtonLength - RCCallInsideMargin * 4 - RCCallLabelHeight -
            (titleY + RCCallLabelHeight + RCCallInsideMargin));

    if (self.userCollectionViewLayout) {
      self.userCollectionView.collectionViewLayout =
          self.userCollectionViewLayout;
    } else {
      RCAudioMultiCallUserCollectionLayout *userCollectionViewLayout =
          [[RCAudioMultiCallUserCollectionLayout alloc] initWithItemMargin:5
                                                             buttomPadding:10];
      [self.userCollectionView setCollectionViewLayout:userCollectionViewLayout
                                              animated:YES];
    }
    self.userCollectionView.hidden = NO;
  } else if (callStatus == RCCallDialing ||
             (callStatus == RCCallActive && !self.isFullScreen)) {
    self.userCollectionView.frame =
        CGRectMake(0, self.view.frame.size.height - RCCallVerticalMargin -
                          RCCallButtonLength * 3.5 - RCCallInsideMargin * 2,
    ...
    self.userCollectionView.frame =
        CGRectMake(0, self.view.frame.size.height - RCCallInsideMargin -
                          RCCallButtonLength * 2.5,
                   self.view.frame.size.width, RCCallButtonLength * 2.5);
    self.userCollectionView.hidden = NO;
  }
...
- (void)remoteUserDidInvite:(NSString *)userId
  ...
  NSString *userName =
      [[RCUserInfoCacheManager sharedManager] getUserInfo:userId].name;
  if (userName) {
    NSString *inviteTips = [NSString
        stringWithFormat:NSLocalizedStringFromTable(@"VoIPCallRemoteDidInvite",
                                                    @"RongCloudKit", nil),
                         userName];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  RCCallMultiCallUserCell *cell = [collectionView
      dequeueReusableCellWithReuseIdentifier:RongVoIPMultiVideoCellReuseID
  ...
  [self.callSession setVideoView:(UIView *)cell.headerImageView
...
- (void)onUserInfoUpdate:(NSNotification *)notification {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    if ([updateUserId isEqualToString:weakSelf.mainModel.userId]) {
      ...
      [weakSelf.inviterPortraitView
          setImageURL:[NSURL URLWithString:updateUserInfo.portraitUri]];
    } else {
      RCCallUserCallInfoModel *model =
          [weakSelf getModelInSubUserModelList:updateUserInfo.userId];
      if (model) {
