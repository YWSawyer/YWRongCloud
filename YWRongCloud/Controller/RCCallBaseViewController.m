...
- (void)onOrientationChanged:(NSNotification *)notification {
  [self resetLayout:(self.callSession.conversationType != ConversationType_PRIVATE)
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  if (self.callSession.callStatus == RCCallActive) {
    ...
    self.tipsLabel.text = NSLocalizedStringFromTable(
    ...
    if (self.callSession.mediaType == RCCallMediaAudio) {
      ...
      self.tipsLabel.text = NSLocalizedStringFromTable(@"VoIPVideoCallIncoming",
                                                       @"RongCloudKit", nil);
    }
...
- (UIButton *)minimizeButton {
  if (!_minimizeButton) {
    _minimizeButton = [[UIButton alloc]
        initWithFrame:CGRectMake(RCCallHorizontalMargin / 2,
                                 RCCallVerticalMargin / 2, RCCallButtonLength,
                                 RCCallButtonLength)];
    [_minimizeButton
        setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/minimize.png"]
...
- (UIButton *)inviteUserButton {
  if (!_inviteUserButton) {
    _inviteUserButton = [[UIButton alloc]
        initWithFrame:CGRectMake(RCCallHorizontalMargin / 2,
                                 RCCallVerticalMargin / 2, RCCallButtonLength,
                                 RCCallButtonLength)];
    [_inviteUserButton
        setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/add.png"]
        forState:UIControlStateNormal];
    [_inviteUserButton
        setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/add.png"]
        forState:UIControlStateHighlighted];
...
- (UIButton *)muteButton {
  if (!_muteButton) {
    ...
    [_muteButton
        setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/mute.png"]
        forState:UIControlStateNormal];
    [_muteButton
        setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/mute_hover.png"]
        forState:UIControlStateHighlighted];
    [_muteButton
        setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/mute_hover.png"]
        forState:UIControlStateSelected];
    [_muteButton setTitle:NSLocalizedStringFromTable(@"VoIPCallMute",
                                                     @"RongCloudKit", nil)
                 forState:UIControlStateNormal];
...
- (UIButton *)speakerButton {
  if (!_speakerButton) {
    ...
    [_speakerButton
        setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/handfree.png"]
        forState:UIControlStateNormal];
    [_speakerButton setImage:[RCCallKitUtility
                                 imageFromVoIPBundle:@"voip/handfree_hover.png"]
                    forState:UIControlStateHighlighted];
    [_speakerButton setImage:[RCCallKitUtility
                                 imageFromVoIPBundle:@"voip/handfree_hover.png"]
                    forState:UIControlStateSelected];
    [_speakerButton setTitle:NSLocalizedStringFromTable(@"VoIPCallSpeaker",
                                                        @"RongCloudKit", nil)
                    forState:UIControlStateNormal];
...
- (UIButton *)acceptButton {
  if (!_acceptButton) {
    ...
    [_acceptButton
        setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/answer.png"]
        forState:UIControlStateNormal];
    [_acceptButton
        setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/answer_hover.png"]
        forState:UIControlStateHighlighted];
    [_acceptButton setTitle:NSLocalizedStringFromTable(@"VoIPCallAccept",
                                                       @"RongCloudKit", nil)
                   forState:UIControlStateNormal];
...
- (UIButton *)hangupButton {
  if (!_hangupButton) {
    ...
    [_hangupButton
        setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/hang_up.png"]
        forState:UIControlStateNormal];
    [_hangupButton setImage:[RCCallKitUtility
                                imageFromVoIPBundle:@"voip/hang_up_hover.png"]
                   forState:UIControlStateHighlighted];
    [_hangupButton setTitle:NSLocalizedStringFromTable(@"VoIPCallHangup",
                                                       @"RongCloudKit", nil)
                   forState:UIControlStateNormal];
  ...
  if (!_cameraCloseButton) {
    ...
    if (self.callSession.conversationType == ConversationType_PRIVATE) {
      ...
      [_cameraCloseButton
          ...
          forState:UIControlStateHighlighted];
      [_cameraCloseButton
          setTitle:NSLocalizedStringFromTable(@"VoIPVideoCallTurnToAudioCall",
                                              @"RongCloudKit", nil)
          forState:UIControlStateNormal];
    } else {
      [_cameraCloseButton
          setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/video.png"]
          forState:UIControlStateNormal];
      [_cameraCloseButton
          setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/video.png"]
          forState:UIControlStateHighlighted];
      [_cameraCloseButton
          setImage:[RCCallKitUtility
                       imageFromVoIPBundle:@"voip/video_hover.png"]
          forState:UIControlStateSelected];
      [_cameraCloseButton
          setTitle:NSLocalizedStringFromTable(@"VoIPVideoCallCloseCamera",
                                              @"RongCloudKit", nil)
          forState:UIControlStateNormal];
      [_cameraCloseButton
          setTitle:NSLocalizedStringFromTable(@"VoIPVideoCallOpenCamera",
                                              @"RongCloudKit", nil)
          forState:UIControlStateSelected];
    }
...
- (UIButton *)cameraSwitchButton {
  if (!_cameraSwitchButton) {
    ...
    if (self.callSession.conversationType == ConversationType_PRIVATE) {
      ...
      [_cameraSwitchButton
          setImage:[RCCallKitUtility
                       imageFromVoIPBundle:@"voip/camera_hover.png"]
          forState:UIControlStateHighlighted];
      [_cameraSwitchButton
          setImage:[RCCallKitUtility
                       imageFromVoIPBundle:@"voip/camera_hover.png"]
          forState:UIControlStateSelected];
      [_cameraSwitchButton
          setTitle:NSLocalizedStringFromTable(@"VoIPAudioCallCamera",
                                              @"RongCloudKit", nil)
          forState:UIControlStateNormal];
      [_cameraSwitchButton
          setTitle:NSLocalizedStringFromTable(@"VoIPAudioCallCamera",
      ...
      [_cameraSwitchButton
          setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/change.png"]
          forState:UIControlStateNormal];
      [_cameraSwitchButton
          setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/change.png"]
          forState:UIControlStateHighlighted];
  ...
  if (mediaType == RCCallMediaAudio &&
    ...
    if (callStatus == RCCallActive) {
      self.minimizeButton.frame =
          CGRectMake(RCCallHorizontalMargin / 2, RCCallVerticalMargin,
    ...
    if (callStatus == RCCallActive) {
      self.timeLabel.frame =
          CGRectMake(RCCallHorizontalMargin,
                     RCCallVerticalMargin * 3 + RCCallHeaderLength +
                         RCCallInsideMargin * 2 + RCCallLabelHeight,
    ...
    if (callStatus == RCCallHangup) {
      self.tipsLabel.frame = CGRectMake(
          ...
          self.view.frame.size.height - RCCallVerticalMargin -
              RCCallButtonLength - RCCallInsideMargin * 3 - RCCallLabelHeight,
          self.view.frame.size.width - RCCallHorizontalMargin * 2,
          RCCallLabelHeight);
      ...
      self.tipsLabel.frame =
          CGRectMake(RCCallHorizontalMargin,
                     MAX((self.view.frame.size.height - RCCallLabelHeight) / 2,
                         RCCallVerticalMargin * 3 + RCCallHeaderLength +
                             RCCallInsideMargin * 3 + RCCallLabelHeight * 2),
                     self.view.frame.size.width - RCCallHorizontalMargin * 2,
                     RCCallLabelHeight);
    } else {
      self.tipsLabel.frame =
    ...
    if (callStatus == RCCallActive) {
      self.muteButton.frame = CGRectMake(
          RCCallHorizontalMargin, self.view.frame.size.height -
                                      RCCallVerticalMargin - RCCallButtonLength,
    ...
    if (callStatus == RCCallActive) {
      self.speakerButton.frame =
          CGRectMake(self.view.frame.size.width - RCCallHorizontalMargin -
                         RCCallButtonLength,
                     self.view.frame.size.height - RCCallVerticalMargin -
                         RCCallButtonLength,
                     RCCallButtonLength, RCCallButtonLength);
    ...
    if (callStatus == RCCallActive) {
      self.timeLabel.frame = CGRectMake(
          RCCallHorizontalMargin,
          RCCallVerticalMargin + RCCallInsideMargin + RCCallLabelHeight,
          self.view.frame.size.width - RCCallHorizontalMargin * 2,
          RCCallLabelHeight);
      self.timeLabel.hidden = NO;
    } else if (callStatus != RCCallHangup) {
    ...
    if (callStatus == RCCallActive) {
      self.tipsLabel.frame =
          CGRectMake(RCCallHorizontalMargin,
                     MAX((self.view.frame.size.height - RCCallLabelHeight) / 2,
                         RCCallVerticalMargin + RCCallHeaderLength * 1.5 +
                             RCCallInsideMargin * 3),
                     self.view.frame.size.width - RCCallHorizontalMargin * 2,
                     RCCallLabelHeight);
      ...
      self.tipsLabel.frame =
          CGRectMake(RCCallHorizontalMargin,
                     self.view.frame.size.height - RCCallVerticalMargin -
                         RCCallButtonLength * 2 - RCCallInsideMargin * 8,
                     self.view.frame.size.width - RCCallHorizontalMargin * 2,
                     RCCallLabelHeight);
    }
    self.tipsLabel.hidden = NO;
    ...
    if (callStatus == RCCallActive) {
      self.cameraCloseButton.frame =
          CGRectMake(self.view.frame.size.width - RCCallHorizontalMargin -
                         RCCallButtonLength,
                     self.view.frame.size.height - RCCallVerticalMargin -
                         RCCallButtonLength * 2 - RCCallInsideMargin * 5,
                     RCCallButtonLength, RCCallButtonLength);
    ...
    if (callStatus == RCCallActive) {
      self.inviteUserButton.frame = CGRectMake(
          self.view.frame.size.width - RCCallHorizontalMargin / 2 -
              RCCallButtonLength / 2,
    ...
    if (callStatus == RCCallIncoming || callStatus == RCCallRinging) {
      self.tipsLabel.frame =
          CGRectMake(RCCallHorizontalMargin,
                     RCCallVerticalMargin * 2 + RCCallHeaderLength +
                         RCCallInsideMargin * 2 + RCCallLabelHeight,
                     self.view.frame.size.width - RCCallHorizontalMargin * 2,
                     RCCallLabelHeight);
    } else if (callStatus == RCCallDialing) {
    ...
    if (callStatus == RCCallActive) {
      self.cameraSwitchButton.frame = CGRectMake(
          self.view.frame.size.width - RCCallHorizontalMargin / 2 -
              RCCallButtonLength - RCCallInsideMargin,
          RCCallVerticalMargin, RCCallButtonLength / 2, RCCallButtonLength / 2);
      self.cameraSwitchButton.hidden = NO;
    } else if (callStatus != RCCallHangup) {
      self.cameraSwitchButton.hidden = YES;
    ...
    if (callStatus == RCCallIncoming || callStatus == RCCallRinging) {
      ...
      self.tipsLabel.frame =
          CGRectMake(RCCallHorizontalMargin,
                     self.view.frame.size.height - RCCallVerticalMargin -
                         RCCallButtonLength * 3.5 - RCCallInsideMargin * 5 -
                         RCCallLabelHeight,
                     self.view.frame.size.width - RCCallHorizontalMargin * 2,
                     RCCallLabelHeight);
    }
...
- (void)layoutTextUnderImageButton:(UIButton *)button {
  ...
  button.imageEdgeInsets = UIEdgeInsetsMake(
      -button.titleLabel.intrinsicContentSize.height - RCCallInsideMargin, 0, 0,
...
- (void)callDidDisconnect {
  ...
  if (self.callSession.connectedTime > 0) {
    ...
    self.tipsLabel.text = [RCCallKitUtility
        getReadableStringForCallViewController:self.callSession
                                                   .disconnectReason];
  }
