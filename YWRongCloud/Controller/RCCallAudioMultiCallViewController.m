...
- (void)setMainModel:(RCCallUserCallInfoModel *)mainModel {
  ...
  if (mainModel.userInfo) {
    ...
    [self.inviterPortraitView
        setImageURL:[NSURL URLWithString:mainModel.userInfo.portraitUri]];
  } else {
    mainModel.userInfo =
        [[RCUserInfoCacheManager sharedManager] getUserInfo:mainModel.userId];
  }
}
...
- (void)resetLayout:(BOOL)isMultiCall
  ...
  if (callStatus == RCCallIncoming || callStatus == RCCallRinging) {
    ...
    self.userCollectionView.frame = CGRectMake(
        0, (self.view.frame.size.height - RCCallButtonLength * 3) / 2,
    ...
    if (self.userCollectionViewLayout) {
      ...
      RCAudioMultiCallUserCollectionLayout *userCollectionViewLayout =
          [[RCAudioMultiCallUserCollectionLayout alloc] initWithItemMargin:2
                                                             buttomPadding:10];
      [self.userCollectionView setCollectionViewLayout:userCollectionViewLayout
                                              animated:YES];
    }
...
- (void)callDidConnect {
  ...
  if (![self.callSession.caller isEqualToString:currentUserId]) {
    [self.subUserModelList
        removeObject:[self getModelInSubUserModelList:currentUserId]];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  RCCallMultiCallUserCell *cell = [collectionView
      dequeueReusableCellWithReuseIdentifier:RongVoIPMultiAudioCellReuseID
